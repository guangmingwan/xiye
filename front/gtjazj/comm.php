...
function replylist($reply_msgtype_id,$reply_list, $fromUsername, $toUsername, $time){
	$resultStr="";
	if($reply_msgtype_id==""){
		$msgType = "text";
		$contentStr="回复类型为空,请核对reply_msgtype_id";
		$resultStr = sprintf(TEXTTPL, $fromUsername, $toUsername, $time, $msgType, $contentStr);
	}
	elseif($reply_msgtype_id=="1"){
		$msgType = "text";
		$contentStr=$reply_list;
		$resultStr = sprintf(TEXTTPL, $fromUsername, $toUsername, $time, $msgType, $contentStr);
	}
	elseif($reply_msgtype_id=="2"){
		$msgType = "music";	
		$Music=explode("|",$reply_list);
		$resultStr = "";
		if(count($Music)==4){
			$resultStr = sprintf(MUSICTPL, $fromUsername, $toUsername, $time, $msgType, $Music[0], $Music[1], $Music[2],$Music[3]);
		}
		else{
			$msgType = "text";
			$contentStr="回复数据结构解析出错,请校验";
			$resultStr = sprintf(TEXTTPL, $fromUsername, $toUsername, $time, $msgType, $contentStr);
		}
	}
	elseif($reply_msgtype_id=="3"){
		$msgType = "news";
		$Article=explode(",,,",$reply_list);
		//print_r($Article);
		$Articlecount=count($Article);
		$ArticleString="";
		for ($i=0;$i<$Articlecount;$i++){
			$Article_array=explode("|||",$Article[$i]);
			//print_r($Article_array);
			$ArticleItemString="";
			if(count($Article_array)==4){
				$ArticleItemString=sprintf(ARTICLEITEM,$Article_array[0], $Article_array[1], $Article_array[2], $Article_array[3]);
			}
			else{
				$msgType = "text";
				$contentStr="回复数据结构解析出错,请校验";
		...
		$msgType = "text";
		$contentStr="该回复类型'$reply_msgtype_id'不存在,请核对reply_msgtype_id";
		$resultStr = sprintf(TEXTTPL, $fromUsername, $toUsername, $time, $msgType, $contentStr);
	}
...
//replylist("3","非农发力，美国QE如期缩减几确定     报告作者： 韦志超,汪进|||非农发力，美国QE如期缩减几确定||||||http://42.121.28.128:8080/gtjazj/reply.html?action_id=19,,,剧烈的去产能已经开始   报告类型： 事件点评 报告作者： 薛鹤翔|||剧烈的去产能已经开始||||||http://42.121.28.128:8080/gtjazj/reply.html?action_id=18,,,汇荒来袭，去产能加速 报告类型： 日报/周报/双周报/月报 报告作者： 薛鹤翔,汪进|||汇荒来袭，去产能加速||||||http://42.121.28.128:8080/gtjazj/reply.html?action_id=17,,,稳增长无力，稳信心缺位    报告作者： 薛鹤翔,汪进|||稳增长无力，稳信心缺位||||||http://42.121.28.128:8080/gtjazj/reply.html?action_id=16,,,市场高估2014年的外需改善   报告作者： 韦志超,汪进|||市场高估2014年的外需改善||||||http://42.121.28.128:8080/gtjazj/reply.html?action_id=15");
