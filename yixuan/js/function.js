				...
				if(retcode=="0"){	
					if (cmd=="querykc"){
						...
						if(today_count<1){
							...
							setCookie_time("today_count",today_count,"h24");
						...
						$("#gsjj").attr("href",xiangxiurl+base64_encode(encodeURIComponent(gsjjurl)));		
						...
						$("#gyzs").attr("href",xiangxiurl+base64_encode(encodeURIComponent(gyzsurl)));		
						...
						$("#kktz").attr("href",xiangxiurl+base64_encode(encodeURIComponent(kktzurl)));		
						...
						$("#zcfw").attr("href",xiangxiurl+base64_encode(encodeURIComponent(zcfwurl)));		
						...
						$("#kcxx").attr("href",xiangxiurl+base64_encode(encodeURIComponent(kcxxurl)));		
						...
						if(extend=="1"){
							...
							for (var i = 0; i < jsonobj.data.length; i++) {
								...
								str+="<div class=\"hua1\">";
								str+="<li>";
								str+="<a target=\"_blank\"  href=\""+jsonobj.data[i].yxzp+"\" title=\""+getyxzptype(jsonobj.data[i].yxzp_type)+"\" style=\"top: 0px; \">";
							...
							var j=1;
							j=Math.ceil(i/16);
							str+="<a id=\"page_before\" onclick=\"getindex('before',"+j+","+i+")\">《上一页</a>";
							for (var k = 1; k <= j; k++) {
								str+="<a id=\"page_"+k+"\" onclick=\"getindex("+k+","+j+","+i+")\">"+k+"</a>";
							}
							str+="<a id=\"page_next\"  onclick=\"getindex('next',"+j+","+i+")\" >下一页》</a>";
							str+="<div>";
						...
						for (var i = 0; i < jsonobj.data.length; i++) {
               			for(var e in jsonobj.data[i]){
							...
							str+="<td width=\"67\" rowspan=\"2\"><input type=\"image\" src=\"images/sc.jpg\" onclick=\"del('wtdy','wtdy_id="+jsonobj.data[i].wtdy_id+"')\"></td>";
							str+="</tr>";
							str+="<tr>";
							str+="<td>答：</td>";
						...
						for (var i = 0; i < jsonobj.data.length; i++) {
               			for(var e in jsonobj.data[i]){
							...
							str+="<td width=\"67\" rowspan=\"2\"><input type=\"image\" src=\"images/sc.jpg\" onclick=\"del('yjfk','yjfk_id="+jsonobj.data[i].yjfk_id+"')\"></td>";
							str+="</tr>";
						}
						...
						for (var i = 0; i < jsonobj.data.length; i++) {
               			for(var e in jsonobj.data[i]){
							...
							str+="<div class=\"tu1\"><div id=\"demo01\" class=\"flexslider\">";
							str+="<ul class=\"slides\">";
...
function newadd(cmd){
	...
	if(cmd=="product_type_zl"){
		...
		if(pic_count>=3){
			...
			str+="<input name=\"files[]\"  type=\"file\" multiple>";
...
function save(cmd){
	if(cmd=="bm"){
		...
		var cz=$("[name='cz']").val();
		var qd=$("[name='qd']").val();	
		main("bm","username="+username+"&sex="+sex+"&birthday="+birthday+"&parent="+parent+"&phone="+phone+"&address="+address+"&email="+email+"&weibo="+weibo+"&qq="+qq+"&nickname="+nickname+"&xgjg="+xgjg+"&xgpxb="+xgpxb+"&xqah="+xqah+"&jrxgzy="+jrxgzy+"&cz="+cz+"&qd="+qd);
	}
...
function init(){
	...
	if(!lxwmurl){
		lxwmurl="http://115.28.39.216:8080/zhengsheng/uploadfile/zcfw/%E6%94%AF%E6%8C%81%E6%9C%8D%E5%8A%A1x.jpg";
		setCookie_time("lxwmurl",lxwmurl,"h2");
	}
	
	$("#lxwm").attr("href",xiangxiurl+base64_encode(encodeURIComponent(lxwmurl)));
